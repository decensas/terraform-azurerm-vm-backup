...
| <a name="input_azure_policy_scope"></a> [azure\_policy\_scope](#input\_azure\_policy\_scope) | (Optional) What scope to assign an Azure policy to assign backup policies to VMs on and the ID of the scope. The key be one of 'management\_group', 'subscription' or 'resource\_group' and the value is the ID of the scope | `map(string)` | `{}` | no |
...
| <a name="input_enable_dynamic_backup_policy_assignment"></a> [enable\_dynamic\_backup\_policy\_assignment](#input\_enable\_dynamic\_backup\_policy\_assignment) | Whether to enable dynamic backup policy to VMs using Azure Policy and tags | `bool` | `false` | no |
...
| <a name="input_immutability"></a> [immutability](#input\_immutability) | Whether you want vault to be immutable. Allowed values are: 'Locked', 'Unlocked' or 'Disabled'. Review https://learn.microsoft.com/en-us/azure/backup/backup-azure-immutable-vault-concept?tabs=recovery-services-vault | `string` | `null` | no |
