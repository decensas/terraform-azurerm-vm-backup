...
Terraform module for deploying Azure Virtual Machine backup using Azure Policy.
...
| <a name="input_protected_virtual_machine_ids"></a> [protected\_virtual\_machine\_ids](#input\_protected\_virtual\_machine\_ids) | (Optional) A map of backup policies mapped to virtual machine IDs. The key is backup policy name while the value is a list of virtual machine IDs. Will assign VMs a backup policy directly. | `map(list(string))` | `{}` | no |
...
- Code Examples
- Support encryption
- Enable backup policy management using Azure policies
